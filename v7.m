...
while q<nq && abs(K1-K0)>tolk
    ...
    % Prices
    r0  = prod_em*alpha*K0^(alpha-1)-delta;
    w0 = prod_em*(1-alpha)*(K0^(alpha));
...
% BSum = sum(BHistM,3);
K1 = max(0.01,(KEM-BorrowSum)/mean(mean(LHist,1)));

%% Display results
disp('  capital-labor ratio');
disp(K1);
