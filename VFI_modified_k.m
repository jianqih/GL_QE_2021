...
while abs(err)>tol_r
                ...
                for a_ub = 1:ABI % abi of unincorporated 
                    for a_ib= 1:ABI
                        for exp_bus=1:EXP_BUS_TYPE_RETIRE
                            futurevalue_retire0 = zeros(nk,1);
                            futurevalue_retire1 = zeros(nk,1);
                            futurevalue_retire2 = zeros(nk,1);
                            for k_next_idx = 1:nk
                                if (t < AGE)
                                    for consump_n_next=1:SHOCK
                                        for consump_i_next=1:SHOCK
                                            futurevalue_retire0(k_next_idx) = futurevalue_retire0(k_next_idx) + BETA * ZETA(t-RETIRE_AGE) * value_retire(t-RETIRE_AGE,e,a_em,a_ub,a_ib,1,k_next_idx,consump_n_next,consump_i_next) * prob_consump_n(consump_n_next) * prob_consump_i(consump_i_next);
                            ...
                            for k = 1:nk % index k 
                                ...
                                cons = kap(k) * (1 + R) + P * W * h_em(a_em, e) * exp((4 * ALPHA_1 + 22.7 * ALPHA_2)); % actural holding
                                if (t == AGE)
                                    value_worker = util(cons);
                                    capitalchoice_worker = 5; % polciy function: capital = 0 
                                else
                                    l = 0;
                                    while l<nk
                                        l=l+1;  % real capital k next period
                                        kap0=kap(k); % current asset holdings
                                        kap1=kap(l); % future asset holdings
                                        value_tmp = util(cons-kap1) + futurevalue_retire0(l);
                                        % disp((k_next - CAP_MIN)/CAP_STEP+1);
                                        if (value_worker < value_tmp)
                                            value_worker = value_tmp;
                                            capitalchoice_worker = l;
                                        end
                                    end
                                end
                                ...
                                if (kap(k) >= 0)
                                    for shock_n = 1:SHOCK
                                        ...
                                        if (invest_non_incorp > (1 + LAMBDA_E) * kap(k))
                                            invest_non_incorp = (1 + LAMBDA_E) * kap(k);
                                        end
                                        ...
                                        if (t == AGE)
                                            ...
                                            while l<nk
                                                ...
                                                kap0=kap(k); % current asset holdings
                                                kap1=kap(l); % future asset holdings
                                                value_tmp = util(cons-kap1) + futurevalue_retire1(l);
                                                % disp((k_next - CAP_MIN)/CAP_STEP+1);
                                                if (value_non_incorp_tmp < value_tmp)
                                                    value_non_incorp_tmp = value_tmp;
                                                    capitalchoice_non_incorp(shock_n) = l;
                                                end
                                            end
                                        end
                                ...
                                if (capital(k) >= 0)
                                    for shock_i=1:SHOCK
                                        ...
                                        k_incorp = (1 - DELTA) * invest_incorp + i_scaler_incorp * power(invest_incorp, NU_IB) - I_COST + (kap(k) - invest_incorp) * (1 + R);
                                        if (exp_bus == 2)
                                            k_incorp = (1 - DELTA) * invest_incorp + i_scaler_incorp * power(invest_incorp, NU_IB) + (kap(k) - invest_incorp) * (1 + R);
                                        end
                                        if (t == AGE)
                                            value_incorp_tmp = util(k_incorp);
                                            capitalchoice_incorp(shock_i) = 1;
                                        else
                                            while l<nk && kap(l)<=kapmax
                                                l = l+1;  % real capital k next period
                                                kap0 = kap(k); % current asset holdings
                                                kap1 = kap(l); % future asset holdings
                                                value_tmp = util(cons-kap1) + futurevalue_retire1(l);
                                                ...
                                                if (value_non_incorp_tmp < value_tmp)
                                                    value_non_incorp_tmp = value_tmp;
                                                    capitalchoice_incorp(shock_i) = l;
                                                end
                                            end                                                                                       
                                        end
                        ...
                        for exp_bus = 1:EXP_BUS_TYPE
                            ...
                            for k_next_idx = 1:nk
                                if (t == RETIRE_AGE)
                                    for consump_n_next = 1:SHOCK
                                        for consump_i_next= 1:SHOCK
                                            futurevalue_retire0(k_next_idx) = futurevalue_retire0(k_next_idx) + BETA * value_retire(1,e,a_em,a_ub,a_ib,1,k_next_idx,consump_n_next,consump_i_next) * prob_consump_n(consump_n_next) * prob_consump_i(consump_i_next);
                                            futurevalue_retire1(k_next_idx) = futurevalue_retire1(k_next_idx) + BETA * value_retire(1,e,a_em,a_ub,a_ib,2,k_next_idx,consump_n_next,consump_i_next) * prob_consump_n(consump_n_next) * prob_consump_i(consump_i_next);
                                            futurevalue_retire2(k_next_idx) = futurevalue_retire2(k_next_idx) + BETA * value_retire(1,e,a_em,a_ub,a_ib,3,k_next_idx,consump_n_next,consump_i_next) * prob_consump_n(consump_n_next) * prob_consump_i(consump_i_next);
                                        end
                                    end
                                else
                                    for consump_n_next = 1:SHOCK
                                        for consump_i_next = 1:SHOCK
                                            futurevalue0(k_next_idx) = futurevalue0(k_next_idx) + BETA * value(t+1,e,a_em,a_ub,a_ib,1,k_next_idx,consump_n_next,consump_i_next) * prob_consump_n(consump_n_next) * prob_consump_i(consump_i_next);
                                            futurevalue1(k_next_idx) = futurevalue1(k_next_idx) + BETA * value(t+1,e,a_em,a_ub,a_ib,2,k_next_idx,consump_n_next,consump_i_next) * prob_consump_n(consump_n_next) * prob_consump_i(consump_i_next);
                                        end
                                    end
                                end
                            end
                            ...
                            for k=1:nk
                                ...
                                for shock_w = 1:SHOCK
                                    ...
                                    k_worker = kap(k) * (1 + R) + W * h_em(a_em, e) * exp((ALPHA_1 * exp_em(t, e) + ALPHA_2 * exp_em(t, e) * exp_em(t, e) + shock_grid_w(e,shock_w)));
                                    for k_next = 0:CAP_STEP:round(min(k_worker,kap(CAP))) % capital cannot negative
                                ...
                                value_worker_noshock(t,e,a_em,a_ub,a_ib,exp_bus,k) = value_worker;
                                value_non_incorp_noshock(t,e,a_em,a_ub,a_ib,exp_bus,k) = value_non_incorp;
                                value_incorp_noshock(t,e,a_em,a_ub,a_ib,exp_bus,k) = value_incorp;

                                for consump_n = 1:SHOCK
                                    for consump_i = 1:SHOCK
                                        if (value_worker > value_non_incorp + consump_grid_n(consump_n) && value_worker > value_incorp + consump_grid_i(consump_i))
                                            value(t,e,a_em,a_ub,a_ib,exp_bus,k,consump_n,consump_i) = value_worker;
                                            career(t,e,a_em,a_ub,a_ib,exp_bus,k,consump_n,consump_i) = WORKER;
                                            for shock_w = 1:SHOCK
                                                capitalchoice(t,e,a_em,a_ub,a_ib,exp_bus,k) = capitalchoice_worker(shock_w);
                                            end
                                        elseif (value_non_incorp + consump_grid_n(consump_n) >= value_incorp + consump_grid_i(consump_i))
                                            value(t,e,a_em,a_ub,a_ib,exp_bus,k,consump_n,consump_i) = value_non_incorp + consump_grid_n(consump_n);
                                            career(t,e,a_em,a_ub,a_ib,exp_bus,k,consump_n,consump_i) = NON_INCORP;
                                            for shock_n = 1:SHOCK
                                                capitalchoice(t,e,a_em,a_ub,a_ib,exp_bus,k,consump_n,consump_i,shock_n) = capitalchoice_non_incorp(shock_n);
                                            end
                                        else
                                            value(t,e,a_em,a_ub,a_ib,exp_bus,k,consump_n,consump_i) = value_incorp + consump_grid_i(consump_i);
                                            career(t,e,a_em,a_ub,a_ib,exp_bus,k,consump_n,consump_i) = INCORP;
                                            for shock_i = 1:SHOCK
                                                capitalchoice(t,e,a_em,a_ub,a_ib,exp_bus,k,consump_n,consump_i,shock_i) = capitalchoice_incorp(shock_i);
                                            end
                                        end
                                    end
                                end
        ...
        for a_ub=1:ABI
            for a_ib=1:ABI
                ...
                for k_next_idx=1:CAP
                    for consump_n_next=1:SHOCK
                        for consump_i_next=1:SHOCK
                            schoolfuture1(k_next_idx) = schoolfuture1(k_next_idx) + BETA * value(2,2,a_em,a_ub,a_ib,1,k_next_idx,consump_n_next,consump_i_next) * prob_consump_n(consump_n_next) * prob_consump_i(consump_i_next);
                            schoolfuture2(k_next_idx) = schoolfuture1(k_next_idx) + BETA * value(2,3,a_em,a_ub,a_ib,1,k_next_idx,consump_n_next,consump_i_next) * prob_consump_n(consump_n_next) * prob_consump_i(consump_i_next);
                        end
                    end
                end
                ...
                for k = 1:nk
                    for k_fam = 1:nk
                        ...
                        for consump_n = 1:SHOCK
                            for consump_i = 1:SHOCK
                                value_highschool = value_highschool + value(1,1,a_em,a_ub,a_ib,1,k,consump_n,consump_i) * prob_consump_n(consump_n) * prob_consump_i(consump_i);
                            end
                        end
                        ...
                        k_college = (1 + R) * (kap(k) - COLLEGE_TUITION + financial_aid_nonelite(k_fam, a_em));
                        for k_next = CAP_MIN:CAP_STEP:min(k_college,kap(CAP)) % allow debt at college
                        ...
                        value_non_elite_noshock(a_em,a_ub,a_ib,k,k_fam) = value_college;
                        value_elite_noshock(a_em,a_ub,a_ib,k,k_fam) = value_elitecollege;
                        ...
                        for consump_c = 1:SHOCK
                            for consump_e = 1:SHOCK
                                if (value_highschool > value_college + consump_grid_c(consump_c) && value_highschool > value_elitecollege + consump_grid_e(consump_e))
                                    schoolchoice(a_em,a_ub,a_ib,k,k_fam,consump_c,consump_e) = NO_COLLEGE;
                                    schoolvalue(a_em,a_ub,a_ib,k,k_fam,consump_c,consump_e) = value_highschool;
                                    schoolcapital(a_em,a_ub,a_ib,k,k_fam,consump_c,consump_e) = -1;
                                elseif (value_college + consump_grid_c(consump_c) > value_elitecollege + consump_grid_e(consump_e))
                                    schoolchoice(a_em,a_ub,a_ib,k,k_fam,consump_c,consump_e) = COLLEGE;
                                    schoolvalue(a_em,a_ub,a_ib,k,k_fam,consump_c,consump_e) = value_college + consump_grid_c(consump_c);
                                    schoolcapital(a_em,a_ub,a_ib,k,k_fam,consump_c,consump_e) = capitalchoice_college;
                                else
                                    schoolchoice(a_em,a_ub,a_ib,k,k_fam,consump_c,consump_e) = ELITE;
                                    ...
                                    schoolvalue(a_em,a_ub,a_ib,k,k_fam,consump_c,consump_e) = ELITE_ADMIT(sat_g) * value_elitecollege + (1-ELITE_ADMIT(sat_g)) * value_college + consump_grid_e(consump_e);
                                    schoolcapital(a_em,a_ub,a_ib,k,k_fam,consump_c,consump_e) = capitalchoice_elitecollege;
                                    if (value_highschool > value_college + consump_grid_c(consump_c))
                                        schoolchoice2(a_em,a_ub,a_ib,k,k_fam,consump_c,consump_e) = NO_COLLEGE;
                                        % no_college = no_college +1;
                                        schoolcapital2(a_em,a_ub,a_ib,k,k_fam,consump_c,consump_e) = -1;
                                    else
                                        schoolchoice2(a_em,a_ub,a_ib,k,k_fam,consump_c,consump_e) = COLLEGE;
                                        % college = college +1;
                                        schoolcapital2(a_em,a_ub,a_ib,k,k_fam,consump_c,consump_e) = capitalchoice_college;
                                    end
                                end
                            end
                        end
    ...
    for id = 1:num_id
        ...
        edu_sim(id) = schoolchoice(a_em,a_ub,a_ib,k,k_fam,consump_c,consump_e);
        % check if the student is admitted by elite college
        flag = 0;
        if (edu_sim(id) == 3)
            sat_group=1;
	        ...
	        if edu_random_number > ELITE_ADMIT(sat_group)
		        edu_sim(id) = schoolchoice2(a_em,a_ub,a_ib,k,k_fam,consump_c,consump_e);
        ...
        if (edu_sim(id) == 1)
            ...
            career_sim(id,t) = career(t,1,a_em,a_ub,a_ib,exp_bus,k,consump_n,consump_i);
            if (career_sim(id,t) == 1)
                ...
                exp_bus_sim(id,t) = 1;
                k_idx_sim(id,t+1) = capitalchoice(t,e,a_em,a_ub,a_ib,exp_bus,k,consump_n,consump_i,shock_w);
            ...
            elseif (career_sim(id,1) == 2)
                ...
                exp_bus_sim(id,1) = 2;
                k_idx_sim(id,t+1) = capitalchoice(t,e,a_em,a_ub,a_ib,exp_bus,k,consump_n,consump_i,shock_n);
                ...
                Borrow_sum = Borrow_sum + invest_non_incorp - capital(k);
            else
                h_em_ = h_em(a_em, e) * exp(ALPHA_1 * exp_em(t, e) + ALPHA_2 * exp_em(t, e) * exp_em(t, e));
                i_scaler_incorp = PROD_I * h_ib(a_ib, e) * exp(shock_grid_i(shock_i)) * power(h_em_, RHO_IB);
                invest_incorp = power((DELTA+R)/(i_scaler_incorp * NU_IB), 1/(NU_IB - 1));
                ...
                exp_bus_sim(id,2) = 3;
                k_idx_sim(id,t+1) = capitalchoice(t,e,a_em,a_ub,a_ib,exp_bus,k,consump_n,consump_i,shock_i);
                ...
                Borrow_sum = Borrow_sum + invest_incorp - capital(k);
            end
        else
            % college
            career_sim(id,1) = 1;
            income_sim(id,1) = 1;
            exp_bus_sim(id,1) = 1;
            k_idx_sim(id,t+1) = schoolcapital(a_em,a_ub,a_ib,k,k_fam,consump_c,consump_e);
            if (flag == 1)
	            k_idx_sim(id,t+1) = schoolcapital2(a_em,a_ub,a_ib,k,k_fam,consump_c,consump_e);
            end
        end
    end
    ...
    for id = 1:num_id
        for t = 2:RETIRE_AGE
            ...
            shock_n = shock_n_sim(id,t);
            shock_i = shock_i_sim(id,t);
            career_sim(id,t) = career(t,e,a_em,a_ub,a_ib,exp_bus,k,consump_n,consump_i);
    ...
    for id = 1:num_id
        for t = RETIRE_AGE+1:AGE            
            ...
            if (shock_s_sim(id,t) < ZETA(t-RETIRE_AGE+1)) % survive or not
                career_sim(id,t) = career_retire(t-RETIRE_AGE+1,e,a_em,a_ub,a_ib,exp_bus,k,consump_n,consump_i);
                if (career_sim(id,t) == 1)
                    income_sim(id,t) = P * W * h_em(a_em, e) * exp(4 * ALPHA_1 + 22.7 * ALPHA_2);
                    exp_bus_sim(id,t) = 1;
                    k_idx_sim(id,t+1) = capitalchoice_retire(t-RETIRE_AGE+1,e,a_em,a_ub,a_ib,exp_bus,k,consump_n,consump_i,shock_w);
                    capital_sum = capital_sum + capital(k_idx_sim(id,t));
                    capitalw_sum = capitalw_sum + capital(k_idx_sim(id,t));
                elseif (career_sim(id,t) == 2)
                    h_em_ = h_em(a_em, e) * exp(ALPHA_1 * exp_em(t, e) + ALPHA_2 * exp_em(t, e) * exp_em(t, e));
                    ...
                    income_sim(id,t) = (1-DELTA) * invest_non_incorp + i_scaler_non_incorp * power(invest_non_incorp, NU_UB) - invest_non_incorp * (1 + R);
                    exp_bus_sim(id,t) = 2;
                    k_idx_sim(id,t+1) = capitalchoice_retire(t-RETIRE_AGE+1,e,a_em,a_ub,a_ib,exp_bus,k,consump_n,consump_i,shock_n);
                    capital_sum = capital_sum + (invest_non_incorp - capital(k_idx_sim(id,t)));                    
                    Borrow_sum = Borrow_sum + invest_non_incorp - capital(k);
                else
                    h_em_ = h_em(a_em, e) * exp(ALPHA_1 * exp_em(t, e) + ALPHA_2 * exp_em(t, e) * exp_em(t, e));
                    ...
                    exp_bus_sim(id,t) = 3;
                    k_idx_sim(id,t+1) = capitalchoice_retire(t-RETIRE_AGE+1,e,a_em,a_ub,a_ib,exp_bus,k,consump_n,consump_i,shock_i);
                    capital_sum = capital_sum + (invest_incorp - capital(k_idx_sim(id,t)));
                    Borrow_sum = Borrow_sum + invest_incorp - capital(k);
                end
                career_sim(id,t) = -1;
    ...
    K_EM = abs(capitalw_sum-Borrow_sum);
    K1 = K_EM/labor_sum;
    err = norm(K1-K);
