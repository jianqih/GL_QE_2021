...
value_retire = ones(cS.AGE-cS.RETIRE_AGE,cS.EDU,ABI,ABI,ABI,EXP_BUS_TYPE_RETIRE,CAP,SHOCK,SHOCK); %# 10 dims
career_retire = ones(cS.AGE-cS.RETIRE_AGE,EDU,ABI,ABI,ABI,EXP_BUS_TYPE_RETIRE,CAP,SHOCK,SHOCK); % 10 dims
capitalchoice_retire = ones(cS.AGE-cS.RETIRE_AGE,EDU,ABI,ABI,ABI,EXP_BUS_TYPE_RETIRE,CAP,SHOCK,SHOCK,SHOCK); %11 dims
...
while abs(err)>tol_r
    ...
    for t = AGE:-1:RETIRE_AGE+1
        ...
        futurevalue_retire = zeros(CAP,3);
        for k_next_idx = 1:CAP
            if (t < AGE)
                futurevalue_retire(k_next_idx,:) = futurevalue_retire(k_next_idx,:) + BETA * ZETA(t-RETIRE_AGE) * value_retire(t-RETIRE_AGE,:,:,:,:,:,k_next_idx,:,:) .* prob_consump_n .* prob_consump_i;
            end
        end
        for k = 1:CAP % index k 
            % worker's decision
            ...
            k_worker = capital(k) * (1 + R) + P * W * h_em * exp((4 * ALPHA_1 + 22.7 * ALPHA_2)); % actural holding
            if (t == AGE)
                ...
                for k_next = 0:CAP_STEP:round(min(k_worker, capital(CAP))) % real capital k next period
                    value_tmp = util(k_worker - k_next) + futurevalue_retire((k_next - CAP_MIN)/CAP_STEP+1);
            ...
            if (capital(k) >= 0)
                for shock_n = 1:SHOCK
                    ...
                    h_em_ = h_em * exp(cS.ALPHA_1 * exp_em(t,:) + cS.ALPHA_2 * exp_em(t,:) .* exp_em(t,:))';
                    i_scaler_non_incorp = cS.PROD_N * (exp(cS.shock_grid_n) .* power(h_em_, RHO_UB)')*h_ub;
                    invest_non_incorp = power((cS.delta+R)./(i_scaler_non_incorp * NU_UB), 1/(NU_UB - 1));
            ...
            if (capital(k) >= 0)
                for shock_i=1:SHOCK
                    ...
                    h_em_ = h_em * exp(ALPHA_1 * exp_em + ALPHA_2 * exp_em * exp_em);
                ...
                for consump_i = 1:SHOCK
                    if (exp_bus > 1)
                        if value_worker > value_non_incorp + consump_grid_n(consump_n) && value_worker > value_incorp + consump_grid_i(consump_i)
                            value_retire(t-RETIRE_AGE,:,:,:,:,:,k,consump_n,consump_i) = value_worker;
                            career_retire(t-RETIRE_AGE,:,:,:,:,:,k,consump_n,consump_i) = WORKER;
                            for shock_w = 1:SHOCK
                                ...
                                if (t == RETIRE_AGE)
                                    for consump_n_next = 1:SHOCK
                                        for consump_i_next= 1:SHOCK
                                            ...
                                            futurevalue_retire1(k_next_idx) = futurevalue_retire0(k_next_idx) + BETA * value_retire(1,e,a_em,a_ub,a_ib,2,k_next_idx,consump_n_next,consump_i_next) * prob_consump_n(consump_n_next) * prob_consump_i(consump_i_next);
                                    ...
                                    for consump_n_next = 1:SHOCK
                                        for consump_i_next = 1:SHOCK
                                            ...
                                            futurevalue1(k_next_idx) = futurevalue0(k_next_idx) + BETA * value(t+1,e,a_em,a_ub,a_ib,2,k_next_idx,consump_n_next,consump_i_next) * prob_consump_n(consump_n_next) * prob_consump_i(consump_i_next);
                                        end
                                    end
                                end
                            end
                                ...
                                for consump_n = 1:SHOCK
                                    for consump_i = 1:SHOCK
                                        if (value_worker > value_non_incorp + consump_grid_n(consump_n) && value_worker > value_incorp + consump_grid_i(consump_i))
                                            ...
                                            for shock_w = 1:SHOCK
                                                capitalchoice(t,e,a_em,a_ub,a_ib,exp_bus,k,consump_n,consump_i,shock_w) = capitalchoice_worker(shock_w);
                                            end
                                        elseif (value_non_incorp + consump_grid_n(consump_n) >= value_incorp + consump_grid_i(consump_i))
    ...
    % schooling decision
    schoolfuture = zeros(CAP,2);
    for k_next_idx=1:CAP
        for consump_n_next=1:SHOCK
            for consump_i_next=1:SHOCK
                schoolfuture(k_next_idx) = schoolfuture1(k_next_idx) + BETA * value(2,2,a_em,a_ub,a_ib,1,k_next_idx,consump_n_next,consump_i_next) * prob_consump_n(consump_n_next) * prob_consump_i(consump_i_next);
            end
        end
    end
    ...
    for id = 1:num_id
        for t = RETIRE_AGE+1:AGE
            ...
            if (shock_s_sim(id,t) < ZETA(t-RETIRE_AGE+1)) % survive or not
                career_sim(id,t) = career_retire(t-RETIRE_AGE,e,a_em,a_ub,a_ib,exp_bus,k,consump_n,consump_i);
                if (career_sim(id,t) == 1)
                    income_sim(id,t) = P * W * h_em(a_em, e) * exp(4 * ALPHA_1(e) + 22.7 * ALPHA_2(e));
                    exp_bus_sim(id,t) = 1;
                    k_idx_sim(id,t+1) = capitalchoice_retire(t-RETIRE_AGE,e,a_em,a_ub,a_ib,exp_bus,k,consump_n,consump_i,shock_w);
                    capitalw_sum = capitalw_sum + capital(k_idx_sim(id,t));
                elseif (career_sim(id,t) == 2)
                    h_em_ = h_em(a_em, e) * exp(ALPHA_1(e) * exp_em(t, e) + ALPHA_2(e) * exp_em(t, e) * exp_em(t, e));
                    i_scaler_non_incorp = PROD_N * h_ub(a_ub, e) * exp(shock_grid_n(shock_n)) * power(h_em_, RHO_UB);
                    ...
                    income_sim(id,t) = (1-DELTA) * invest_non_incorp + i_scaler_non_incorp * power(invest_non_incorp, NU_UB) - invest_non_incorp * (1 + R);
                    exp_bus_sim(id,t) = 2;
                    k_idx_sim(id,t+1) = capitalchoice_retire(t-RETIRE_AGE,e,a_em,a_ub,a_ib,exp_bus,k,consump_n,consump_i,shock_n);
                    capital_sum = capital_sum + (invest_non_incorp - capital(k_idx_sim(id,t)));
                else
                    h_em_ = h_em(a_em, e) * exp(ALPHA_1(e) * exp_em(t, e) + ALPHA_2(e) * exp_em(t, e) * exp_em(t, e));
                    i_scaler_incorp = PROD_I * h_ib(a_ib, e) * exp(shock_grid_i(shock_i)) * power(h_em_, RHO_IB);
                    ...
                    exp_bus_sim(id,t) = 3;
                    k_idx_sim(id,t+1) = capitalchoice_retire(t-RETIRE_AGE,e,a_em,a_ub,a_ib,exp_bus,k,consump_n,consump_i,shock_i);
                    capital_sum = capital_sum + (invest_incorp - capital(k_idx_sim(id,t))); %invest - holding = borrow
                end
            end
        end
    end
    % **********************************Calculate production function********************************************************************   
    K1 = abs(capitalw_sum - capital_sum);
    R1 = PROD_EM*(K1/labor_sum)^(1-alpha)*alpha;
    W = PROD_EM*((K1/labor_sum)^(alpha))*(1-alpha);

    err = norm(R1-R);
    if err > 0 % R1>R new value higher than before
        maxrate = R1;
    else
        minrate = R1;
    end
    disp(['k1 = ',num2str(K1),', r0 = ',num2str(R),', r1 = ',num2str(R1), ', err = ', num2str(err)])
