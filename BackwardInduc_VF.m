function valueM_ts = BackwardInduc_VF(valueM_LastT,r,w,cS,BP,EP)
%% Documentation
%{
...
cPolM    = ones(cS.nk*cS.ns, cS.nk, EP-1); 
...
kM        = zeros(size(kChoiceM,1),size(kChoiceM,2),EP-1);
for t = BP:EP-1
...
%% Backward Induction
valueM_ts           = zeros(size(valueM_end,1),size(valueM_end,2),EP);
valueM_ts(:,:,EP) = valueM_LastT;
Ind_ts              = zeros(cS.nk,cS.ns,Ts.T-1);

% Preparation
walue  = zeros(size(uM));
valueM = zeros(cS.nk*cS.ns,1,EP-1);
Ind    = zeros(cS.nk*cS.ns,1,EP-1);
vNext  = zeros(cS.ns, cS.nk, EP-1);
